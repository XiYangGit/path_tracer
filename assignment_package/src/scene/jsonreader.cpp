...
#include <scene/geometry/surfaces.h>
...
#include <scene/lights/projectionlight.h>
#include <iostream>
#include <scene/materials/homogeneousmedium.h>
...
//        shape = std::make_shared<Mesh>();
    ...
    else if(QString::compare(type, QString("Surface")) == 0)
    {
        QString surfacetype;
        if(geometry.contains(QString("type")))
        {
            surfacetype=geometry["type"].toString();
            std::cout<<"load surface type: "<<surfacetype.toStdString()<<std::endl;
        }else
            std::cout<<"unrecognized surface type!!"<<std::endl;
        shape = std::make_shared<Surface>(surfacetype);
...
//        shape = std::make_shared<Mesh>();
    ...
    else if(QString::compare(lgtType, QString("PointLight")) == 0)
    {
        Color3f lightColor = ToVec3(geometry["lightColor"].toArray());
        Float intensity = static_cast< float >(geometry["intensity"].toDouble());
        Point3f lightPos = ToVec3(geometry["position"].toArray());
        lightType = std::make_shared<PointLight>(shape->transform,lightPos, lightColor * intensity);
    ...
    else if(QString::compare(lgtType, QString("SpotLight")) == 0)
    {
        ...
        Float intensity = static_cast< float >(geometry["intensity"].toDouble());
        Point3f lightPos = ToVec3(geometry["position"].toArray());
        lightType = std::make_shared<SpotLight>(shape->transform,lightPos,lightColor * intensity,totalWidth,falloffStart);
    ...
    else if(QString::compare(lgtType, QString("ProjectionLight")) == 0)
    {
        std::shared_ptr<QImage> projectionMap;
        if(geometry.contains(QString("projectionMap"))) {

            QString img_filepath = local_path.toString().append(geometry["projectionMap"].toString());
            projectionMap = std::make_shared<QImage>(img_filepath);
        }
        Color3f lightColor = ToVec3(geometry["lightColor"].toArray());
        Float intensity = static_cast< float >(geometry["intensity"].toDouble());
        Point3f lightPos = ToVec3(geometry["position"].toArray());
        Float fov = static_cast< float >(geometry["fov"].toDouble());
        lightType = std::make_shared<ProjectionLight>(shape->transform,projectionMap,lightPos, lightColor * intensity,fov);
...
Camera JSONReader::LoadCamera(QJsonObject& camera)
{
    ...
    if(camera.contains(QString("medium"))) {
        QJsonObject mediumObj = camera["medium"].toObject();
        QString type;
        if(mediumObj.contains(QString("type"))){
            type = mediumObj["type"].toString();
        }
        if(QString::compare(type, QString("HomogeneousMedium")) == 0)
        {
            Color3f sigma_a(0.0f),sigma_s(0.0f);
            Float g(0.0f);
            if(mediumObj.contains(QString("sigma_a"))) sigma_a = ToVec3(mediumObj["sigma_a"].toArray());
            if(mediumObj.contains(QString("sigma_s"))) sigma_s = ToVec3(mediumObj["sigma_s"].toArray());
            if(mediumObj.contains(QString("g"))) g = mediumObj["g"].toDouble();
            medium=new HomogeneousMedium(sigma_a,sigma_s,g);
            std::cout<<"Load medium type: "<<"HomogeneousMedium "<<g<<std::endl;
        }

    }
    Camera result(medium);
