...
Camera::Camera(Medium *medium):
    Camera(400, 400, Vector3f(0,0,10), Vector3f(0,0,0), Vector3f(0,1,0),medium)
{}
Camera::Camera(unsigned int w, unsigned int h):
    Camera(w, h, Vector3f(0,0,10), Vector3f(0,0,0), Vector3f(0,1,0),nullptr)
{}
...
//    eye(e),
//    ref(r),
//    world_up(worldUp),medium(nullptr)
//{
//    RecomputeAttributes();
//}
Camera::Camera(unsigned int w, unsigned int h, const Vector3f &e, const Vector3f &r, const Vector3f &worldUp):
    Camera(w,h,e,r,worldUp,nullptr)
{
}
Camera::Camera(unsigned int w, unsigned int h, const Vector3f &e, const Vector3f &r, const Vector3f &worldUp, Medium *medium):
    fovy(45),
    width(w),
    height(h),
    near_clip(0.1f),
...
Ray Camera::RaycastNDC(float ndc_x, float ndc_y) const
{
    ...
    Ray result(eye, glm::normalize(P - eye),medium);
    return result;
}
...
Ray LensBasedCamera::Raycast(const Point2f &pt, const Point2f &sample) const
{
    Ray ray=Camera::Raycast(pt.x, pt.y);
    //Modify ray for depth of field
    if (lensRadius > 0.0f) {
    //Sample point on lens 374
        Point3f pLens = lensRadius * WarpFunctions::squareToDiskConcentric(sample);
    //Compute point on plane of focus 375
        Float ft = (focalDistance-ray.origin.z) / ray.direction.z;
        Point3f pFocus = ray.origin+ray.direction*ft;
    //Update ray for effect of lens 375
        ray.origin = ray.origin+pLens;
        ray.direction = glm::normalize(pFocus - ray.origin);
    }
    return Ray(ray.origin,ray.direction,medium);
}
