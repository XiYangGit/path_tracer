...
Color3f DiffuseAreaLight::Sample_Li(const Intersection &ref, const Point2f &xi, Vector3f *wi, Float *pdf, Intersection *lightsample) const
{
    Intersection isect=shape->Sample(ref,xi,pdf);
    *lightsample=isect;
...
bool DiffuseAreaLight::getPosition(Point3f *wi) const
{
    return false;
}
