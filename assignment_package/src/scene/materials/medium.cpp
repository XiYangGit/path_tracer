...
bool GetMediumScatteringProperties(const std::string &name, Color3f *sigma_a,
                                   Color3f *sigma_prime_s) {
    for (MeasuredSS &mss : SubsurfaceParameterTable) {
        if (name == mss.name) {
            *sigma_a = Color3f(mss.sigma_a[0],mss.sigma_a[1],mss.sigma_a[2]);
            *sigma_prime_s = Color3f(mss.sigma_prime_s[0],mss.sigma_prime_s[1],mss.sigma_prime_s[2]);
            return true;
        }
    }
    return false;
